...
> **Semantic note:** Offboard normally means "a companion computer is flying this." Here an onboard module is filling that role. That is unusual but entirely legitimate — `offboard_control_mode` is a uORB topic, and Commander does not care whether it arrived from a MAVLink link, a ROS 2 node, or a module running on the same chip.
...
| `COM_OBL_RC_ACT` | Altitude (or Land) | What to do on offboard loss. **Not** Return to Launch — this platform has no GPS |
...
4. **Written analysis (2–3 pages):**
   ...
   - Comparison of your gains against Lab 2's autotune results — where they differ and why
